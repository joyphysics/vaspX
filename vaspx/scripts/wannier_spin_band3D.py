...
files_to_copy = ["wannier90.up.win", "wannier90.dn.win", "wannier90.up.amn", "wannier90.dn.amn", "wannier90.up.mmn", "wannier90.dn.mmn", "wannier90.up.eig", "wannier90.dn.eig", "POSCAR"]
...
3     # Appropriate options as needed
"""  
...
wannier90_up_win_path = os.path.join(output_dir, "wannier90.up.win")
...
if begin_block is not None and end_block is not None:
    ...
    with open(wannier90_up_win_path, "r") as win_file:
        lines = win_file.readlines()

    # Write back with the kpoint block inserted after "end atoms_cart"
    with open(wannier90_up_win_path, "w") as win_file:
        for line in lines:
            win_file.write(line)
...
grep_process = subprocess.Popen(
    ['grep', 'NBANDS', '2_SCF/OUTCAR'],  # Corrected the path to the OUTCAR file
    stdout=subprocess.PIPE,
    stderr=subprocess.PIPE,
    universal_newlines=True
)
...
# Step 6: Modify the wannier90.dn.win file in the 11_wannier_band folder
wannier90_dn_win_path = os.path.join(output_dir, "wannier90.dn.win")
...
if begin_block is not None and end_block is not None:
    ...
    # Read the current lines of wannier90.win
    with open(wannier90_dn_win_path, "r") as win_file:
        lines = win_file.readlines()

    # Write back with the kpoint block inserted after "end atoms_cart"
    with open(wannier90_dn_win_path, "w") as win_file:
        for line in lines:
            win_file.write(line)
            if "end atoms_cart" in line:
                win_file.write("\nbands_plot      =  true\n")
                win_file.write("bands_plot_format = gnuplot xmgrace\n")
                win_file.write("bands_num_points = 40\n\n")
                win_file.write("dis_num_iter = 1000\n")
                win_file.write("dis_win_min  = \n")
                win_file.write("dis_win_max  = \n")
                win_file.write("dis_froz_max = \n\n")
                win_file.write("write_xyz = true \n")
...
# Step 7: Extract the value of NBANDS from OUTCAR using grep
grep_process = subprocess.Popen(
    ['grep', 'NBANDS', '2_SCF/OUTCAR'],  # Corrected the path to the OUTCAR file
    stdout=subprocess.PIPE,
    stderr=subprocess.PIPE,
...
print("wannier_band directory created successfully and wannier90.up/dn.win modified. You can now proceed with your calculations.")
